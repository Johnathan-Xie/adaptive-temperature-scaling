...
from llmtuner.model.calibration import CalibrationConfig, CalibrationModelForCausalLM, CALIBRATION_WEIGHTS_NAME, CALIBRATION_SAFETENSORS_WEIGHTS_NAME
if TYPE_CHECKING:
...
def load_model_and_tokenizer(
    ...
    stage: Optional[Literal["pt", "sft", "rm", "ppo", "calibration"]] = "sft"
    ...
    if finetuning_args.finetuning_type == "lora" or model_args.is_calibration_model or model_args.checkpoint_dir is None:
    ...
    # Fix config (for Qwen)
    if "qwen" in getattr(config, "model_type", None):
        for dtype_name, dtype in [("fp16", torch.float16), ("bf16", torch.bfloat16), ("fp32", torch.float32)]:
            setattr(config, dtype_name, getattr(config, "torch_dtype", None) == dtype)
        # Issue with Qwen KV Cache. Not entirely sure why.
    ...
    if model_args.is_calibration_model:
        model = CalibrationModelForCausalLM.from_pretrained(
    ...
    if model_args.calibration_config_path is not None:
        calibration_config_update_keys = ["loss_type", "label_smoothing", "smooth_loss_weight", "smoothing_topk", "label_smoothing_type", "log_auxiliary_info"]
        model_args_dict = model_args.to_dict()
        calibration_config_updates = {k: model_args_dict[k] for k in calibration_config_update_keys if model_args_dict[k] is not None}
        calibration_config = CalibrationConfig.from_pretrained(
            model_args.calibration_config_path, base_model_name_or_path=model_to_load, **calibration_config_updates)
        
        model = CalibrationModelForCausalLM(model, calibration_config)
